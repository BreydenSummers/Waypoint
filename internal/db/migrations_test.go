...
func TestExportLifecycleMigrationsUpgradeAndRollbackOnRealPostgreSQL(t *testing.T) {
	...
	mustReject(t, db, `UPDATE export_job SET bundle_archive_path = '   ' WHERE id = $1`, baseJobID)
	...
	mustReject(t, db, `INSERT INTO export_receipt (id, export_job_id, engagement_id, status, bundle_path, archive_byte_length, archive_sha256, manifest_sha256, cutoff, verified_at, verified_by, verifier_version, revision) VALUES ($1, $2, $3, 'verified', '   ', 10, $4, $5, now(), now(), $6, 'waypoint-verify/1.0.0', 1)`, "88888888-8888-4888-8888-888888888887", baseJobID, engagementID, strings.Repeat("1", 64), strings.Repeat("2", 64), actorID)
	...
	mustReject(t, db, `INSERT INTO teardown_authorization (id, engagement_id, receipt_id, export_job_id, bundle_path, archive_sha256, manifest_sha256, requested_by, requested_at, expires_at, status, revision) VALUES ($1, $2, $3, $4, '   ', $5, $6, $7, now(), now() + interval '5 minutes', 'authorized', 1)`, "88888888-8888-4888-8888-88888888888a", engagementID, receiptID, completedJobID, strings.Repeat("1", 64), strings.Repeat("2", 64), actorID)
